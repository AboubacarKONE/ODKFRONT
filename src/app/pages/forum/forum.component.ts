...
export class ForumComponent implements OnInit, OnDestroy {
  ...
  constructor(private authenticationService: AuthenticationService, private catForumService: CategoryForumService,
    ...
    private responseService: ResponseService, private logService: LogService) { }
  ...
  onNewCatForum(catForm: NgForm) {
    ...
    formData.append('libelleCat', catForm.value.libelle);
    ...
    this.subscriptions.push(
      this.catForumService.addNewCatForum(formData).subscribe(
        (response: categoryForum) => {
          this.log.action = `ajout de la categorie ${response.libelleCat.substring(0,50)}`;
    ...
    this.subscriptions.push(
      this.catForumService.getAllCategoryForum().subscribe(
        (response: categoryForum[]) => {
          ...
          this.cateForum.forEach(c => {
            // this.auditService.listAudit.push(c)
          })
    ...
    if (this.idCat != null) {
      ...
      this.subscriptions.push(
        this.quizService.addNewQuizForum(formData).subscribe(
          (response: quizForum) => {
            this.log.action = `question ${response.description.substring(0,80)}`;
            this.log.tableName = audit.AJOUTER
            this.log.createdBy = this.user;
            this.logService.saveLog(this.log).subscribe(
              (audit: Log) => {
    ...
    this.subscriptions.push(
      this.responseService.addNewResponseForum(formData).subscribe(
        (response: responseForum) => {
          this.log.action = `reponse ${response.quizForum.categoryForum.libelleCat}`;
          this.log.tableName = audit.AJOUTER
          this.log.createdBy = this.user;
          this.logService.saveLog(this.log).subscribe(
            (audit: Log) => {
    ...
    this.subscriptions.push(
      this.quizService.getAllQuizForum().subscribe(
        (response: quizForum[]) => {
          ...
          this.quizForum.forEach(q => {
            // this.auditService.listAudit.push(q)
          })
          // console.log(this.quizForum);
        },
        (errorResponse: HttpErrorResponse) => {
    ...
    this.subscriptions.push(
      this.responseService.findAllresponseByQuiz(id).subscribe(
        (response: responseForum[]) => {
          ...
          // this.auditService.responseAudit = this.responseForum;
          this.responseForum.forEach(r => {
            // this.auditService.listAudit.push(r)
          })
  ...
  public onUpdateQuiz() {
    ...
    this.subscriptions.push(
      this.quizService.updateQuiz(formData).subscribe(
        (response: quizForum) => {
          this.log.action = `ajout de question pour categorie ${response.categoryForum.libelleCat}`;
  ...
  public onUpdateResponse() {
    ...
    this.subscriptions.push(
      this.responseService.updateResponse(formData).subscribe(
        (response: responseForum) => {
          this.log.action = `reponse à la question pour categorie ${response.quizForum.categoryForum.libelleCat}`;
          this.log.tableName = audit.AJOUTER
          this.log.createdBy = this.user;
          this.logService.saveLog(this.log).subscribe(
            (audit: Log) => {
    ...
    this.subscriptions.push(
      this.catForumService.deleteCatForm(Idcat).subscribe(
        (response: CustomHttpRespone) => {
          this.log.action = `Suppression de la categorie id: ${Idcat}`;
  ...
  onDeleteQuizForum(IdQuiz: number) {
    this.subscriptions.push(
      this.quizService.deleteQuizForm(IdQuiz).subscribe(
        (response: CustomHttpRespone) => {
          this.log.action = `Suppression de la question id:${IdQuiz} `;
          this.log.tableName = audit.SUPPRIMER
          this.log.createdBy = this.user;
          this.logService.saveLog(this.log).subscribe(
            (audit: Log) => {
  ...
  onDeleteResponseForum(IdResponse: number) {
    this.subscriptions.push(
      this.responseService.deleteResponseForm(IdResponse).subscribe(
        (response: CustomHttpRespone) => {
          this.log.action = `Suppression de la question id:${IdResponse} `;
          this.log.tableName = audit.SUPPRIMER
          this.log.createdBy = this.user;
          this.logService.saveLog(this.log).subscribe(
            (audit: Log) => {
